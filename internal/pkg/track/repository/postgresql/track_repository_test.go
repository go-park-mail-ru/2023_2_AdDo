...
func TestTrackRepository_getWithQuery(t *testing.T) {
	...
	const TrackIntDuration = 100

	query := "select id, name, preview, content, duration, lyrics, artist.id, artist_name from track"
	rows := pgxmock.NewRows([]string{"id", "name", "preview", "content", "duration", "lyrics", "artist.id", "artist_name"}).
		AddRow(expected[0].Id, expected[0].Name, expected[0].Preview, expected[0].Content, TrackIntDuration,
			expected[0].Lyrics, expected[0].ArtistId, expected[0].ArtistName)
