...
func (useCase Default) generateAvatarName(id string, extansion string) string {
	data := []byte(id + time.Now().Format("20060102150405"))
	hash := md5.Sum(data)
	hashString := hex.EncodeToString(hash[:])
	return hashString + "." + extansion
}

func (useCase Default) GetAvatar(id string, src io.Reader, size int64) (avatar.Avatar, error) {
