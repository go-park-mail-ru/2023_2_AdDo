...
func (p Postgres) GetByArtistId(artistId uint64) ([]album.Base, error) {
	...
	query := "select album.id, name, preview from album join artist_album on album.id = artist_album.album_id where artist_id = $1"
...
func (p Postgres) Search(text string) ([]album.Base, error) {
	...
	return p.getWithQuery(context.Background(), query, text, album.LimitForMainPage)
}
...
func (p Postgres) Get(albumId uint64) (album.Base, error) {
	...
	if err := p.Pool.QueryRow(context.Background(), query, albumId).Scan(&result.Id, &result.Name, &result.Preview); err != nil {
		p.logger.WithFields(logrus.Fields{
			"err":      err,
			"album id": albumId,
			"query":    query,
