...
func (p Postgres) getWithQuery(ctx context.Context, query string, args ...any) ([]album.Base, error) {
	...
	for rows.Next() {
		...
		err = rows.Scan(&base.Id, &base.Name, &base.Preview, &base.IsSingle)
		if err != nil {
			p.logger.WithFields(logrus.Fields{
				"err":      err,
				"album id": base.Id,
...
func (p Postgres) GetByTrackId(trackId uint64) ([]album.Base, error) {
	...
	query := "select album.id, name, preview, is_single from album join album_track on album.id = album_track.album_id where album_track.track_id = $1"
...
func (p Postgres) GetByArtistId(artistId uint64) ([]album.Base, error) {
	...
	query := "select album.id, name, preview, is_single from album join artist_album on album.id = artist_album.album_id where artist_id = $1"
	return p.getWithQuery(context.Background(), query, artistId)
}

func (p Postgres) GetByUserId(userId string) ([]album.Base, error) {
	p.logger.Infoln("Album Repo GetByArtistId entered")
	query := "select album.id, name, preview, is_single from album join profile_album on album.id = profile_album.album_id where profile_id = $1"
...
func (p Postgres) GetByReleaseDate(limit uint32) ([]album.Base, error) {
	...
	query := "select id, name, preview, is_single from album order by year desc limit $1"
...
func (p Postgres) GetRandom(limit uint32) ([]album.Base, error) {
	...
	query := "select id, name, preview, is_single from album order by random() limit $1"
	return p.getWithQuery(context.Background(), query, limit)
}

func (p Postgres) GetByListenCount(limit uint32) ([]album.Base, error) {
	p.logger.Infoln("Album Repo GetByListenCount entered")
	query := "select album.id, album.name, album.preview, album.is_single from album inner join album_track on album_track.album_id = album.id inner join track on track_id = track.id group by album.id order by sum(track.play_count) desc limit $1"
	return p.getWithQuery(context.Background(), query, limit)
}

func (p Postgres) GetByLikeCount(limit uint32) ([]album.Base, error) {
	p.logger.Infoln("Album Repo GetByLikeCount entered")
	query := "select album.id, name, preview, is_single from album left join profile_album on album.id = profile_album.album_id group by album.id order by count(*) desc limit $1"
	return p.getWithQuery(context.Background(), query, limit)
}

func (p Postgres) Search(text string) ([]album.Base, error) {
	p.logger.Infoln("Album Repo Search entered")
	query := `select album.id, name, preview, is_single from album 
...
func (p Postgres) Get(albumId uint64) (album.Base, error) {
	...
	query := "select id, name, preview, is_single from album where id = $1"
	if err := p.Pool.QueryRow(context.Background(), query, albumId).Scan(&result.Id, &result.Name, &result.Preview, &result.IsSingle); err != nil {
		p.logger.WithFields(logrus.Fields{
			"err":      err,
			"album id": albumId,
			"query":    query,
...
func (p Postgres) CreateLike(userId string, albumId uint64) error {
	...
	if _, err := p.Pool.Exec(context.Background(), query, userId, albumId); err != nil {
		p.logger.WithFields(logrus.Fields{
			"err":      err,
			"album id": albumId,
			"query":    query,
...
func (p Postgres) CheckLike(userId string, albumId uint64) (bool, error) {
	...
	if err := p.Pool.QueryRow(context.Background(), query, userId, albumId).Scan(&counter); err != nil {
