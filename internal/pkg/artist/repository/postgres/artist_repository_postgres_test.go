...
func TestArtistRepository_gettingById(t *testing.T) {
	...
	expectedArtists := []artist.Base{
		{
			Id:     1,
			Name:   "ArtistName",
			Avatar: "Url to artist name image",
		},
	}

	t.Run("Get", func(t *testing.T) {
		profileTable := pgxmock.NewRows([]string{"id", "name", "image"}).
			AddRow(expectedArtists[0].Id, expectedArtists[0].Name, expectedArtists[0].Avatar)

		artistId := uint64(1)
		query := "select artist.id, name, image from artist where artist.id = ?"
	...
	t.Run("GetByTrackId", func(t *testing.T) {
		...
		trackId := uint64(1)
		query := "select artist.id, name, image from artist join artist_track on artist.id = artist_track.artist_id where artist_track.track_id = ?"
	...
	t.Run("GetByAlbumId", func(t *testing.T) {
		...
		albumId := uint64(1)
		query := "select artist.id, artist.name, image from artist join album on artist.id = album.artist_id where album.id = ?"
