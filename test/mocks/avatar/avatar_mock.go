...
import (
	io "io"
	avatar "main/internal/pkg/image"
	reflect "reflect"
...
func (m *MockUseCase) GetAvatar(id string, src io.Reader, size int64) (avatar.Base, error) {
	m.ctrl.T.Helper()
	ret := m.ctrl.Call(m, "GetAvatar", id, src, size)
	ret0, _ := ret[0].(avatar.Base)
	ret1, _ := ret[1].(error)
	return ret0, ret1
}
...
func (m *MockRepository) UploadAvatar(avatar avatar.Base) (string, error) {
	m.ctrl.T.Helper()
...
func (m *MockRepository) UploadPlaylistImage(avatar avatar.Base) (string, error) {
	m.ctrl.T.Helper()
